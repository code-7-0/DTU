/****************************************************************************
 * 版权信息：（C）2017，深圳煜昇科技有限公司版权所有
 *
 * 系统名称：WPP SSR(V1.00)
 *
 * 文件名称：global.c
 *
 * 文件说明: 公用函数文件
 *    
 *         
 * 其他说明：
 *
 * 作    者：
 *
 * 版本信息：V1.00
 *
 * 设计日期：2017-6-28
 *
 ***************************************************************************/
#include "global.h"

/* CRC 高位字节值表 */
const unsigned char CRCHi[256] = {
	
	};
	
/* CRC 低位字节值表 */
const unsigned char CRCLo[256] = {
	
};
unsigned short CRC16(unsigned char *CRCData,unsigned short DataLentch)
{		                                   
    unsigned char uchCRCHi = 0xFF ;  				// high byte of CRC initialized      
    unsigned char uchCRCLo = 0xFF ;  				// low byte of CRC initialized       
    unsigned short  uIndex  ;          				// will index into CRC lookup table  
    while (DataLentch--)             				// Pass through message buffer       
    {
        uIndex = uchCRCHi ^ *CRCData++ ; 			// calculate the CRC          
        uchCRCHi = uchCRCLo ^ CRCHi[uIndex] ;
        uchCRCLo = CRCLo[uIndex] ;
    }
    return (((unsigned short)(uchCRCLo) << 8) | uchCRCHi) ;
}

unsigned short CheckSum(unsigned char *ChkBuf, unsigned short DataLen)
{
	unsigned short chkSum=0;
	
	for(int i=0;i<DataLen;i++)
	{
		chkSum += *(ChkBuf+i);
	}
	return chkSum;
}

unsigned char BCD2HEX(unsigned char bcd_data)    //BCD转为HEX子程序    
{   
    unsigned char temp;   
    temp=(bcd_data/16*10 + bcd_data%16);   
    return temp;   
}   
unsigned char HEX2BCD(unsigned char hex_data)    //HEX转为BCD子程序     
{   
    unsigned char temp;   
    temp=(hex_data/10*16 + hex_data%10);   
    return temp;   
}   

void delay_us(u16 time)  
{
    u16 i=0;

     while(time--)
	 {
		i=6; 
		while(i--);
     }
}

void delay_ms(u16 time)
{    
   u16 i=0;  
   while(time--)
   {
      i=8000;  
      while(i--) ;    
   }
}

//float Int2Float(int val)
//{
//	unFloatInt unFI;
//	unsigned short usTmp,usTmp1;
//	
//	usTmp = (val>>16)&0x0000FFFF;
//	usTmp1 = val&0x0000FFFF;
//	usTmp = ((usTmp<<8)&0xFF00) | ((usTmp>>8)&0x00FF);
//	usTmp1 = ((usTmp1<<8)&0xFF00) | ((usTmp1>>8)&0x00FF);
//	unFI.l = (((u32)usTmp1<<16)&0xFFFF0000) + usTmp;

//	return unFI.f;
//}

