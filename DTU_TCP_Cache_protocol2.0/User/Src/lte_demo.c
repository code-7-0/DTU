...
uint8_t json_to_command(const char *json_string) {
	 uint8_t state=0;
...
//    char *msg_copy = strdup(messages); // 创建消息的副本
    ...
    while (line != NULL && index < MAX_MESSAGES) {
        if (strstr(line, "property/post_reply") != NULL) {//改变数值主题
							const char *json_start = strchr(line, '{');
							if (json_start != NULL) { 
									//提取json语句中的键值 
										uint8_t result = extract_realtime_set(json_start);//时间校准解析
										if(result==0)
											result=json_to_command(json_start);//cjson解析 
