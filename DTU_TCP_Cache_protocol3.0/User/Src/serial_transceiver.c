...
void USART_DMAstart(void)
{
	...
	Gateway_Usart_Read(g_tModule.Gateway_RevBuf,GATEWAY_SIZE_MAX);
...
void Gateway_Usart_Send(uint8_t *buf,uint16_t len)
{ 
		
//		memcpy(g_tModule.Gateway_SendBuf,buf,len);
		HAL_UART_Transmit_DMA(&huart2, buf,len);
//		if(debug_onoff.EC600N)
			debug_printf("send:");
			SendDataAsHex(buf,len);	//debug输出
...
//		HAL_DMA_Abort(&hdma_usart2_rx);
		...
		g_tModule.Gateway_BufLen =  GATEWAY_SIZE_MAX - temp; 
		g_tModule.Gateway_RevFlag = 1;

		Gateway_Usart_Read((uint8_t *)g_tModule.Gateway_RevBuf,GATEWAY_SIZE_MAX);
	 }
...
//		 Ec600_trans_Complete=1;
	 ...
	 { 
		...
		if(huart->Instance==USART2)//如果是串口 gateway
	 { 
        HAL_GPIO_WritePin(RS485_EN2_GPIO_Port,RS485_EN2_Pin,GPIO_PIN_RESET);//开启rs485数据接收	 
	 }	 
